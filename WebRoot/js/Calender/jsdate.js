...
function SelectDate(obj, strFormat, align, vertical) {
    var date = new Date();
    var by = date.getFullYear() - 20;  //最小值 → 20 年前
...
Calendar.prototype.bindData = function () {
    ...
    for (var i = 0; i < tds.length; i++) {
        ...
        //    if (dateArray[i] != "&nbsp;"&&new Date().format(calendar.dateFormatStyle) >=  //将if此处注释可以现在今天以后的日期  修改：王森峰 2011-04-22
        //          new Date(calendar.date.getFullYear(),
        //                   calendar.date.getMonth(),
        //                   dateArray[i]).format(calendar.dateFormatStyle)){

        if (dateArray[i] != "&nbsp;") {    //判断是否选择的地方没有日期   修改：王森峰  2011-07-30
...
Calendar.prototype.getAbsPoint = function (e) {
    var x = e.offsetLeft, y = e.offsetTop, n_clientWidth = e.clientWidth;

    if (cal.align && cal.align == 'right') {
        x -= (this.panel.offsetWidth - n_clientWidth);
    }

    if (cal.vertical && cal.vertical == 'top')
        y -= (this.panel.offsetHeight + e.clientHeight);
...
Calendar.prototype.show = function (dateObj, popControl) {
    ...
    if (brs(navigator.userAgent).browser == 'msie' && brs(navigator.userAgent).version == '8.0')//判断浏览器是否是ie8   王森峰  2011-09-23
    {
        if (this.container.getElementsByTagName("iframe").length > 0) {
            this.container.getElementsByTagName("iframe")[0].style.display = "none";
        }
    }

    dateObj.onblur = function () {if ($(dateObj).val().length > 0 && !IsCDate($(dateObj).val(), 1, 0)) {$(dateObj).val(''); jAlertDialogs("请输入合法的日期格式！");return false; $(dateObj).focus(); } else { calendar.onblur(); } }
...
function brs(ua) {
    ...
    var match = /(webkit)[ \/]([\w.]+)/.exec(ua) ||
        /(msie) ([\w.]+)/.exec(ua) ||
        !/compatible/.test(ua) && /(mozilla)(?:.*? rv:([\w.]+))?/.exec(ua) || /(opera)(?:.*version)?[ \/]([\w.]+)/.exec(ua) ||
        [];
    /* 
...
document.write('<div id="ContainerPanel" style="display:none"><div id="calendarPanel" style="position: absolute;display: none;z-index: 99999;');
